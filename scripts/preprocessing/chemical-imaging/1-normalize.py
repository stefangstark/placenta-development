...
class QC:
    def __init__(self, image, regions, config):
    ...
    def qc_fgmask(self, outpath, image, keep, parts):
        ...
        fig, axes = plt.subplots(2, 2, figsize=viz.scale_figsize(2, 2))
        ax = axes[0, 0]
        sns.histplot(
            x=df["pos"] - df["neg"],
            hue=df["region"],
...
def main(path, config, debug=False):
    ...
    qcer = QC(image, qc.get_regions(sample), config)

    keep, parts = pp.signal_to_noise(image.values, image.wn, **config["mask_fg"])
    qcer.qc_fgmask(qcdir / f"{sample}-fgmask.png", image, keep, parts)
    ...
    qcer.finalize_pipeline(qcdir / f"{sample}-pipeline.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--debug", action="store_true", help="Just run QC pipeline")
